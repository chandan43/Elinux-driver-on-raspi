...
#define GPIO_OE           0x04
#define GPIO_SETDATAOUT   0x1c
#define GPIO_CLEARDATAOUT 0x28
#define GPIO_DATAIN       0x34
...
static void __led_init(void)
{
	...
	ledinit = 1;
	writel_relaxed(data , gbank_base + GPIO_OE);
	data = 1;
	writel_relaxed(data, gbank_base + GPIO_OE);
	return;
}
static ssize_t led_read(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	if (!ledinit)
		LED_INIT;
	data = readl_relaxed(gbank_base + GPIO_DATAIN);
...
static ssize_t led_write(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if(d==0) {
		...
		writel_relaxed(data, gbank_base + GPIO_CLEARDATAOUT);
	}else{
		pr_info("%s: led is on\n",__func__);
		data = data | (1U << gpbank->pinno);
		writel_relaxed(data, gbank_base + GPIO_SETDATAOUT);
	}
	return data;
}
...
static int onboardled_probe(struct platform_device *pdev)
{
	...
	gbank_base = ioremap(gpbank->gpio_base, 64); /* ioremap(unsigned long port, unsigned long size)*/	
	if(!gbank_base)
		return -1;
...
{
	sysfs_remove_file(kobj, &led_attr.attr);
	kobject_put(kobj);
...
{
	pr_info("My first LED driver on RaspberryPi board\n");	
...
MODULE_DESCRIPTION("platform driver for off-board led device on RaspberryPi ");
MODULE_VERSION(".1");
